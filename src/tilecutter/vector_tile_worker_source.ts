...
import {OverzoomedVectorTile, sliceTileLayer, toVirtualVectorTile} from './overzoomed-geojson-vector-tile';
import type {
    WorkerSource,
...
export class VectorTileWorkerSource implements WorkerSource {
    ...
    overzoomedTilesCache: BoundedLRUCache<string, VectorTile>;
    ...
    constructor(actor: IActor, layerIndex: StyleLayerIndex, availableImages: Array<string>) {
        ...
        this.loading = {};
        this.loaded = {};
        this.overzoomedTilesCache = new BoundedLRUCache<string, VectorTile>(10000);
    }
    ...
    private _getOverzoomTile(params: WorkerTileParameters, maxZoomVectorTile: VectorTile): LoadVectorTileResult {
        ...
        if (cachedOverzoomTile) {
            return toVirtualVectorTile(cachedOverzoomTile);
        }
        ...
        for (const sourceLayerId in layerFamilies) {
            ...
            const slicedLayer = sliceTileLayer(sourceLayer, maxZoomTileID, tileID.canonical);
            if (slicedLayer.length > 0) {
                overzoomedVectorTile.addLayer(slicedLayer);
            }
        }

        this.overzoomedTilesCache.set(cacheKey, overzoomedVectorTile);
